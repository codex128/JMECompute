...
public class GLComputeShader extends NativeObject {
     ...
     *
     * @param work amount of work for the compute shader to perform, or null
     * to use the default work size
     */
    public void execute(WorkSize work) {
        assert work != null : "Compute work cannot be null.";
        define("COMPUTE_SHADER", 1);
        define("LOCAL_SIZE_X", work.getLocalX());
        define("LOCAL_SIZE_Y", work.getLocalY());
        define("LOCAL_SIZE_Z", work.getLocalZ());
        updateUniformDefines();
        //parseRuntimeHints(); // todo: totally replace this system
        compileSource(work);
        glUseProgram(id);
        GLRenderUtils.get().setCurrentProgram(id);
        updateUniforms();
        glProgramParameteri(id, GL_COMPUTE_WORK_GROUP_SIZE, 3);
        glDispatchCompute(work.getGlobalX(), work.getGlobalY(), work.getGlobalZ());
